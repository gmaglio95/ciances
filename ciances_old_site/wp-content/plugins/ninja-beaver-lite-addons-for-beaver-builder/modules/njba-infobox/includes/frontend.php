<div class="njba-infobox-sub-main <?php if($settings->image_type == 'icon') echo "njba-infobox-icon-set";  ?>">
    <?php
    ...
    <div class="njba-infobox-contant">
        <?php if($settings->heading_prefix != '') { echo "<".$settings->prefix_tag_selection." class=\"heading_prefix\">".$settings->heading_prefix."</".$settings->prefix_tag_selection.">"; }?>
        <?php if($settings->title != '') { echo "<".$settings->title_tag_selection." class=\"heading\">".$settings->title."</".$settings->title_tag_selection.">"; }?>
        <?php if($settings->text != '') { echo $settings->text; }?>
        <?php if($settings->cta_type != 'none') { ?>
            <?php if($settings->cta_type === 'link') { ?>
                <a href="<?php echo $settings->link;?>" class="njba-infobox-link" target="<?php echo $settings->link_target; ?>"><?php echo $settings->button_text;?></a>
            <?php } ?>
