	...
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main .njba-pricing-inner-heading h1 {
		...
		<?php if( $settings->price_box_layout == '1' || $settings->price_box_layout == '2' || $settings->price_box_layout == '3' ): ?>
			<?php if($settings->heading_margintb['top'])  {?> margin-top: <?php echo $settings->heading_margintb['top'];  ?>px;<?php } ?>
    		<?php if($settings->heading_margintb['bottom'])  {?> margin-bottom: <?php echo $settings->heading_margintb['bottom'];  ?>px;<?php } ?>
    	<?php endif; ?>
    	<?php if ($settings->price_box_layout == '4' || $settings->price_box_layout == '5' ): ?>
    		<?php if($settings->heading_paddingtb['top'])  {?> padding-top: <?php echo $settings->heading_paddingtb['top'];  ?>px;<?php } ?>
    		<?php if($settings->heading_paddingtb['bottom'])  {?> padding-bottom: <?php echo $settings->heading_paddingtb['bottom'];  ?>px;<?php } ?>
    		margin:0;
    	<?php endif; ?>
		
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main .njba-pricing-inner-heading h2 {
		<?php if( $settings->price_box_layout == '1' || $settings->price_box_layout == '2' || $settings->price_box_layout == '3' ): ?>
			<?php if($settings->price_margintb['top'])  {?> margin-top: <?php echo $settings->price_margintb['top'];  ?>px;<?php } ?>
    		<?php if($settings->price_margintb['bottom'])  {?> margin-bottom: <?php echo $settings->price_margintb['bottom'];  ?>px;<?php } ?>
    	<?php endif; ?>
    	<?php if( $settings->price_box_layout == '4' || $settings->price_box_layout == '5' ): ?>
    		<?php if($settings->price_paddingtb['top'])  {?> padding-top: <?php echo $settings->price_paddingtb['top'];  ?>px;<?php } ?>
    		<?php if($settings->price_paddingtb['bottom'])  {?> padding-bottom: <?php echo $settings->price_paddingtb['bottom'];  ?>px;<?php } ?>
    		margin:0;
    	<?php endif; ?>
		<?php if( $settings->price_font['family'] != 'Default' ) { ?><?php FLBuilderFonts::font_css( $settings->price_font ); ?><?php } ?>
		<?php if( $settings->price_font_size ) { ?>font-size: <?php echo $settings->price_font_size; ?>px;<?php } ?>
	} 
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main .njba-pricing-inner-heading span {
		...
		<?php if( $settings->duration_font_size ) { ?>font-size: <?php echo $settings->duration_font_size; ?>px;<?php } ?>
		text-transform: uppercase;
		<?php if( $settings->price_box_layout == '1' || $settings->price_box_layout == '2' || $settings->price_box_layout == '3' ): ?>
			<?php if($settings->duration_margintb['top'])  {?> margin-top: <?php echo $settings->duration_margintb['top'];  ?>px;<?php } ?>
    		<?php if($settings->duration_margintb['bottom'])  {?> margin-bottom: <?php echo $settings->duration_margintb['bottom'];  ?>px;<?php } ?>
    	<?php endif; ?>
    	<?php if( $settings->price_box_layout == '4' || $settings->price_box_layout == '5' ): ?>
    		<?php if($settings->duration_paddingtb['top'])  {?> padding-top: <?php echo $settings->duration_paddingtb['top'];  ?>px;<?php } ?>
    		<?php if($settings->duration_paddingtb['bottom'])  {?> padding-bottom: <?php echo $settings->duration_paddingtb['bottom'];  ?>px;<?php } ?>
    		margin:0;
    	<?php endif; ?>
    	display: inline-block;
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-inner-body {
		<?php if( $settings->properties_border_width ) { ?>border-top: <?php echo $settings->properties_border_width; ?>px;<?php } ?>
		<?php if( $settings->properties_border_color ) { ?>border-color: #<?php echo $settings->properties_border_color; ?>;<?php } ?>
    	<?php if( $settings->properties_border_style ) { ?>border-top-style: <?php echo $settings->properties_border_style; ?>;<?php } ?>
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main .njba-pricing-inner-body ul li {
  		<?php if( $settings->properties_font_size ) { ?>font-size: <?php echo $settings->properties_font_size; ?>px;<?php } ?>
  		<?php if( $settings->properties_font['family'] != 'Default' ) { ?><?php FLBuilderFonts::font_css( $settings->properties_font ); ?><?php } ?>
  		<?php if( $settings->price_box_layout == '1' || $settings->price_box_layout == '2' || $settings->price_box_layout == '3' ): ?>
  			<?php if($settings->properties_margintb['top'])  {?> margin-top: <?php echo $settings->properties_margintb['top'];  ?>px;<?php } ?>
    		<?php if($settings->properties_margintb['bottom'])  {?> margin-bottom: <?php echo $settings->properties_margintb['bottom'];  ?>px;<?php } ?>
    	<?php endif; ?>
    	<?php if( $settings->price_box_layout == '4' || $settings->price_box_layout == '5' ): ?>
    		<?php if($settings->properties_paddingtb['top'])  {?> padding-top: <?php echo $settings->properties_paddingtb['top'];  ?>px;<?php } ?>
    		<?php if($settings->properties_paddingtb['bottom'])  {?> padding-bottom: <?php echo $settings->properties_paddingtb['bottom'];  ?>px;<?php } ?>
    		margin:0;
    	<?php endif; ?>
	}
<?php
for($i=0; $i < count($settings->price_box_content); $i++) :
...
<?php if( $settings->price_box_layout == '1' ): ?>
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-table {padding:0 15px;}
	...
	?>
		.fl-node-<?php echo $id; ?> .layout-1 .njba-label-holder {
			...
			<?php if( $box_content->feature_background_color ) { ?>border-top-color: #<?php echo $box_content->feature_background_color; ?>;<?php } ?>
			font-size: 12px;
			height: 0;
			position: absolute;
			right: 15px;
		...
		<?php if($box_content->featured_item == 'feau_icon') :?>
			.fl-node-<?php echo $id; ?> .layout-1 .njba-label-holder i {
				<?php if( $box_content->feature_color ) { ?>color: #<?php echo $box_content->feature_color; ?>;<?php } ?>
				<?php if( $box_content->feature_font_size ) { ?>font-size: <?php echo $box_content->feature_font_size; ?>px;<?php } ?>
				position: relative;
				right: 30px;
				top: -55px;
				height: auto;
		...
		<?php if($box_content->featured_item == 'feau_text') :?>
			.fl-node-<?php echo $id; ?> .layout-1 .njba-label-holder .njba-text span {
				<?php if( $box_content->feature_color ) { ?>color: #<?php echo $box_content->feature_color; ?>;<?php } ?>
				<?php if( $settings->feature_font['family'] != 'Default' ) { ?><?php FLBuilderFonts::font_css( $settings->feature_font ); ?><?php } ?>
				<?php if( $box_content->feature_font_size ) { ?>font-size: <?php echo $box_content->feature_font_size; ?>px;<?php } ?>
				display: block;
				height: 30px;
				left: -59px;
	...
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-column-<?php echo $i; ?> .njba-pricing-inner-heading h2 {
		<?php if( $box_content->price_color ) { ?>color: #<?php echo $box_content->price_color; ?>;<?php } ?>
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-column-<?php echo $i; ?> .njba-pricing-inner-heading span {
		<?php if( $box_content->duration_color ) { ?>color: #<?php echo $box_content->duration_color; ?>;<?php } ?>
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-column-<?php echo $i; ?> .njba-pricing-inner-body ul li {
		<?php if( $box_content->properties_color ) { ?>color: #<?php echo $box_content->properties_color; ?>;<?php } ?>
	}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-column-<?php echo $i; ?> .njba-pricing-inner-body{
			<?php if( $box_content->foreground ) { ?>background-color: rgba(<?php echo implode(',', FLBuilderColor::hex_to_rgb($box_content->foreground )) ?>, <?php echo $box_content->foreground_opc/100; ?>); <?php } ?>
			overflow: hidden;
		}
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-1 .njba-pricing-column-<?php echo $i; ?> .njba-pricing-inner-heading{
		<?php if( $box_content->foreground_heading ) { ?>background-color: rgba(<?php echo implode(',', FLBuilderColor::hex_to_rgb($box_content->foreground_heading )) ?>, <?php echo $box_content->foreground_opc_heading/100; ?>); <?php } ?>
		overflow: hidden;
	}
	<?php endfor; ?>
<?php endif; ?>
<?php if( $settings->price_box_layout == '2' ): ?>
	.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-2 .njba-pricing-table {padding:0;}
	...
	<?php for($i=0; $i < count($settings->price_box_content); $i++) : ?>
		...
		?>
			.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-2 .njba-pricing-column-<?php echo $i; ?> .njba-label-holder {
				<?php if( $box_content->feature_background_color ) { ?>background-color: #<?php echo $box_content->feature_background_color; ?>;<?php } ?>
				left: -70px;
				padding: 1px 40px;
				position: absolute;
				top: 18px;
			...
			<?php if($box_content->featured_item == 'feau_text') :?>
				.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-2 .njba-pricing-column-<?php echo $i; ?> .njba-label-holder .njba-text span {
					<?php if( $box_content->feature_color ) { ?>color: #<?php echo $box_content->feature_color; ?>;<?php } ?>
					<?php if( $box_content->feature_font->family != 'Default' ) { ?> font-family : "<?php echo $box_content->feature_font->family; ?>";<?php } ?>
					<?php if( $box_content->feature_font->weight != 'Default' ) { ?> font-weight : <?php echo $box_content->feature_font->weight; ?>;<?php } ?>
					<?php if( $box_content->feature_font_size ) { ?>font-size: <?php echo $box_content->feature_font_size; ?>px;<?php } ?>
					<?php if( $box_content->feature_background_color ) { ?>background-color: #<?php echo $box_content->feature_background_color; ?>;<?php } ?>
					display: block;
					height: 30px;
...
<?php if( $settings->price_box_layout == '3' ): ?>
	...
	<?php for($i=0; $i < count($settings->price_box_content); $i++) : ?>
		...
		.fl-node-<?php echo $id; ?> .njba-pricing-table-main.layout-3 .njba-pricing-inner.njba-pricing-column-<?php echo $i; ?>{
			<?php if( $box_content->foreground ) { ?>background-color: #<?php echo $box_content->foreground; ?>;<?php } ?>
		}
