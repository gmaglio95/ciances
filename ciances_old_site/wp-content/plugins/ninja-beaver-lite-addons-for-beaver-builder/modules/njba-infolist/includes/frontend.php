...
<?php
	...
	for($i=0; $i< $total_infolist; $i++){
		...
		?>
		<div class="<?php echo $njba_column; ?> njba-infolist-sec <?php if($box_content->image_type == 'icon') echo "njba-infolist-icon-set ";?><?php echo "njba-infolist-list-".$i; ?>">
		    ...
		    <div class="njba-infolist-content">
		        <?php if($box_content->title != '') { echo "<".$settings->title_tag_selection." class=\"heading\">".$box_content->title."</".$settings->title_tag_selection.">"; }?>
		        <?php if($box_content->text != '') { echo $box_content->text; }?>
		    </div>
		    <?php
