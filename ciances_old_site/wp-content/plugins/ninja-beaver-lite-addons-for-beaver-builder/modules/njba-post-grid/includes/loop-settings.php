...
<div id="fl-builder-settings-section-filter" class="fl-builder-settings-section">
	...
	<?php foreach(FLBuilderLoop::post_types() as $slug => $type) : ?>
		<table class="fl-form-table fl-loop-builder-filter fl-loop-builder-<?php echo $slug; ?>-filter njba-filter-deactive " <?php if($slug == $settings->post_type) echo 'style="display:block;"'; ?>>
		<?php
		// Posts
	...
	FLBuilder::render_settings_field('users', array(
		...
		'help'          => __('Enter a list of authors usernames. Only posts with these authors will be shown.', 'bb-njba')
	), $settings);
	?>
	</table>
</div>
<div id="fl-builder-settings-section-meta" class="fl-builder-settings-section">
	...
	FLBuilder::render_settings_field('show_post_taxonomies', array(
		'type'          => 'select',
		'label'         => __('Show Taxonomy', 'bb-njba'),
		'default'       => '0',
		'options'       => array(
			'1'           => __('Yes', 'bb-njba'),
			'0'           => __('No', 'bb-njba')
	...
	FLBuilder::render_settings_field('post_taxonomies', array(
		'type'          => 'select',
		'label'         => __('Select Taxonomy', 'bb-njba'),
		'default'       => 'none',
		'options'       => array(
...
<script type="text/javascript">
	;(function($) {
		$('.fl-builder-njba-post-grid-settings select[name="post_type"]').on('change', function() {
			var post_type_slug 		= $(this).val();
			var post_taxonomies 	= $('.fl-builder-njba-post-grid-settings select[name="post_taxonomies"]');
		...
		$(document).ready( function() {
			...
			$('.fl-builder-njba-post-grid-settings select[name=post_type]').on('click', function() {
                var selected_value = $(this).find(':selected').val();
                   	$(".njba-filter-deactive").hide();
                    $('.fl-builder-njba-post-grid-settings table.fl-form-table.fl-loop-builder-filter.fl-loop-builder-' + selected_value + '-filter').show();
            });
        });
	})(jQuery);
</script>