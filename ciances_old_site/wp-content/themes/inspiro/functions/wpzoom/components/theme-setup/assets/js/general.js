...
(function ($, _, zoomData) {
    ...
    var themeSetup = {
        ...
        appendSetupButton: function() {
            ...
            var cookieSetupComplete = $.cookie(zoomData.theme_raw_name + '_' + zoomData.demo_imported + "_theme_setup_is_complete");
            var cookieDemoContentDone = $.cookie(zoomData.theme_raw_name + '_' + zoomData.demo_imported + "_demo_content_done") || $.cookie(zoomData.theme_raw_name + '_' + zoomData.theme_raw_name + "_demo_content_done");
        ...
        checkDoneSteps: function(){
            var cookieStepsStatus = $.cookie(zoomData.theme_raw_name + '_' + zoomData.demo_imported + "_steps_status");

            if ( cookieStepsStatus ) {
                $.each( cookieStepsStatus, function(index, status) {
        ...
        updateStepStatus: function(index, status) {
            ...
            zoomData.steps_status = cookieStepsStatus ? cookieStepsStatus : {};

            if ( typeof index != 'undefined' && status != '' ) {
                checkStatus = cookieStepsStatus ? _.contains(['skipped', 'current'], cookieStepsStatus[index]) && status === 'done' : true;
                ...
                if ( ! _.isObject(cookieStepsStatus) || ! cookieStepsStatus[index] || checkStatus ) {
                    zoomData.steps_status[index] = status;
                    $.cookie(zoomData.theme_raw_name + '_' + zoomData.demo_imported + "_steps_status", zoomData.steps_status, { expires: 30 });
                }
            }
        ...
        isComplete: function() {
            ...
            // Set cookie
            // expires in 30 days
            $.cookie(zoomData.theme_raw_name + '_' + zoomData.demo_imported + "_theme_setup_is_complete", 1, { expires : 30 });
