...
.fl-node-<?php echo $id; ?> .njba-highlight-box-main{
    <?php if( $settings->box_bg_color ) { ?>background-color: rgba(<?php echo implode(',', FLBuilderColor::hex_to_rgb($settings->box_bg_color)) ?>, <?php echo $settings->box_bg_opacity/100; ?>); <?php } ?>
    <?php if ( !empty($settings->padding['top']) ) { ?> padding-top: <?php echo $settings->padding['top']; ?>px; <?php } ?>
    <?php if ( !empty($settings->padding['right']) ) { ?> padding-right: <?php echo $settings->padding['right']; ?>px; <?php }  ?>
    <?php if ( !empty($settings->padding['bottom']) ) { ?> padding-bottom: <?php echo $settings->padding['bottom']; ?>px; <?php } ?>
    <?php if ( !empty($settings->padding['left']) ) { ?> padding-left: <?php echo $settings->padding['left']; ?>px; <?php }  ?>
        
    
    <?php if( $settings->box_icon_transition_duration ) { ?>transition: <?php echo 'all ease '.$settings->box_icon_transition_duration;?>s;<?php } ?>
}
    
  <?php   if( $settings->hover_effect != '6' ) { ?>
.fl-node-<?php echo $id; ?> .njba-highlight-box-main:hover .njba-hover-box {
    <?php if( $settings->box_icon_transition_duration ) { ?>transition: <?php echo 'all ease '.$settings->box_icon_transition_duration;?>s;<?php } ?>
    <?php if( $settings->box_bg_hover_color ) { ?>background-color: rgba(<?php echo implode(',', FLBuilderColor::hex_to_rgb($settings->box_bg_hover_color)) ?>, <?php echo $settings->box_bg_hover_opacity/100; ?>); <?php } ?>
}
<?php } ?>
.fl-node-<?php echo $id; ?> .njba-highlight-box-main:hover{
    <?php if( $settings->box_hover_bg_color ) { ?>background-color: rgba(<?php echo implode(',', FLBuilderColor::hex_to_rgb($settings->box_hover_bg_color)) ?>, <?php echo $settings->box_hover_bg_opacity/100; ?>); <?php } ?>
}